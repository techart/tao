...
class DB_ORM_SQL implements Core_ModuleInterface
{
	...
	{
		return new DB_ORM_SQL_Update($table);
	}
	...
	{
		return new DB_ORM_SQL_Delete($table);
	}
	...
	implements Core_StringifyInterface
{
	...
	{
		$this->make_part($name);
		foreach (Core::normalize_args($args) as $v)
			$this->parts[$name][] = (string)$v;
		return $this;
	}
...
class DB_ORM_SQL_Update extends DB_ORM_SQL_Selection
{
	...
	public function as_string()
	{
		...
		if (isset($this->parts['where'])) {
			$where = implode(') AND (', $this->parts['where']);
		}
...
class DB_ORM_SQL_Select extends DB_ORM_SQL_Selection
{
	...
	public function columns()
	{
		...
		foreach (Core::normalize_args(func_get_args()) as $k => $v) {
			if (is_array($v)) {
				foreach ($v as $col)
					$this->parts['columns'][$col] = "$k.$col";
			} else {
				if (is_int($k) && !preg_match('!distinct!i', $v)) {
					$this->parts['columns'][preg_match('{[0-9a-zA-Z_]+\.([0-9a-zA-Z_]+)}', $v, $m) ? $m[1] : $v] = $v;
				} else {
					$this->parts['columns'][$k] = $v;
				}
			}
		}
		return $this;
	...
	public function from()
	{
		...
		foreach (Core::normalize_args(func_get_args()) as $k => $v)
			if (is_int($k)) {
				$this->parts['from'][] = (string)$v;
			} else {
				$this->parts['from'][$k] = (string)$v;
			}
	...
	public function as_string()
	{
		...
		if (isset($this->parts['columns'])) {
			$columns = '';
			foreach ($this->parts['columns'] as $k => $v)
				$columns .= ($columns ? ', ' : '') . ($v == $k ? $this->escape($v) : $this->escape($v) . ' ' . $this->escape($k));
		}

		$from = '';
		foreach ($this->parts['from'] as $k => $v)
			$from .= ($from ? ', ' : '') . $this->escape($v) . (is_int($k) ? '' : " $k");
		...
		$group_by = '';
		if (isset($this->parts['group_by'])) {
			$group_by = implode(', ', $this->parts['group_by']);
		}

		$order_by = '';
		if (isset($this->parts['order_by'])) {
			$order_by = implode(', ', $this->parts['order_by']);
		}

		$having = '';
		if (isset($this->parts['having'])) {
			$having = implode(') AND (', $this->parts['having']);
		}

		$range = '';
		if (isset($this->parts['range'])) {
			$range = 'LIMIT ' . $this->parts['range'][0];
			if ($this->parts['range'][1] > 0) {
				$range .= ' OFFSET ' . $this->parts['range'][1];
			}
		}

		$index = '';
		if (isset($this->parts['index'])) {
			$index = 'USE INDEX (' . $this->parts['index'] . ')';
		}
