...
class DB_ORM implements Core_ModuleInterface, Core_ConfigurableModuleInterface
{
	 ...
	 * @param boolean       $spawn
	 */
	static public function SQLMapper($parent = null, $spawn = false)
	{
		$parent = $parent ? $parent : WS::env()->orm;
		return Core::make('DB.ORM.SQLMapper', $parent, $spawn);
	}
	 ...
	 * @param boolean       $spawn
	 */
	static public function UnionMapper($parent = null, $spawn = false)
	{
		$parent = $parent ? $parent : WS::env()->orm;
		return Core::make('DB.ORM.UnionMapper', $parent, $spawn);
	}
}
	...
	Core_CallInterface
{
	 ...
	 * @return mixed
	 */
	protected function __map($name, $args = null)
	{
		return null;
	}
	...
	Core_IndexedAccessInterface
{
	...
	public function offsetGet($index)
	{
		...
		switch ($index) {
			...
			case 'aliased_table':
				return $this->has_option($index) ? $this->options[$index] : implode(' ', $this['table']);
			case 'table_prefix':
			...
			case 'result':
				...
				if ($only = $this['only']) {
					$r = array_intersect_key($r, array_flip($only));
				}

				foreach ($this['exclude'] as $c)
					unset($r[$c]);
			...
			default:
				...
				if (is_array($value)) {
					return array_merge($parent ? (array)$parent[$index] : array(), $value);
				}
				if (is_null($value)) {
	...
	public function offsetExists($index)
	{
		switch ($index) {
			...
			case 'aliased_table':
				return $this->has_option($index) || isset($this['table']);
			case 'table_prefix':
	...
	/**
	 * Копирует опции из другого маппера или коллекции опций
	 * 
	 * @param DB_ORM_Mapper|DB_ORM_MappingOptions|array $options маппер или коллекция опций
	 * @param array $exclude_options массив опций которые не нужно копировать
	 */
	public function copy_options_from($options, $exclude_options = array())
	{
		$options = is_object($options) ? $options->all_options() : $options;
		foreach($options as $name => $value) {
			if (!in_array($name, $exclude_options)) {
	 ...
	 * @return self
	 */
	protected function array_option($name, $value, $idx = null, $prevent_duplicates = true)
	{
		if ($prevent_duplicates && isset($this->options[$name]) && is_array($this->options[$name]) && array_search($value, $this->options[$name], true) !== false) {
			return $this;
		}
		is_null($idx) ?
			$this->options[$name][] = $value :
	 ...
	 */
	private function use_validator(Validation_Validator $validator)
	{
		return $this->option('validator', $validator);
	}
	...
	Core_StringifyInterface
{
	...
	/**
	 * @see DB_ORM_MappingOptions::all_options
	 * @return array
	 */
	public function all_options()
	{
		return $this->options->all_options();
	}

	/**
	 * @see DB_ORM_MappingOptions::copy_options_from
	 * @param DB_ORM_Mapper|DB_ORM_MappingOptions|array $options
	 * @param array $exclude_options
	 * @return DB_ORM_MappingOptions
	 */
	public function copy_options_from($options, $exclude_options = array())
	{
		return $this->options->copy_options_from($options, $exclude_options);
	}
...
//TODO: название поля id брать из опций маппера
	...
	public function map_dissociated_with($mapper, $entity)
	{
		return $this->where(sprintf('id NOT IN (SELECT %1$s.%2$s_id FROM %1$s WHERE %1$s.%3$s_id = :%3$s_id)',
				$this->associate_table_for($mapper), $this->options['table'][0], self::table_from($mapper)
			), self::id_from($entity)
		);
	}
	...
	{
		$columns = is_array($columns) ? $columns : array((string) $columns);
		$query = array();
		$var_name = uniqid('__like_val');
		foreach ($columns as $i => $col) {
			$query[] = "({$this->options['table_prefix']}.{$col} LIKE :{$var_name})";
		}
		return $this->spawn()->where(implode(' OR ', $query), array($var_name => $value));
	}
	...
	public function stat($just_count = false)
	{
		...
		return Core::with_index(reset($rows->getArrayCopy()), 'count');
	}
	...
	public function find()
	{
		...
		if (($entity = $this->make_cursor($this->sql()->find())->
				...
				execute()->
				fetch()) && method_exists($entity, 'after_find')
		) {
			$entity->after_find();
		}
	...
	public function lookup($value)
	{
		if (($entity = isset($this->options['lookup_by']) ?
				...
				null) && method_exists($entity, 'after_find')
		) {
			$entity->after_find();
		}
	...
	public function update($e, $columns = array(), $key_values = array())
	{
		...
		$rc =
			...
			$this->raw_update($e, $m, $columns, $key_values) &&
	...
	public function raw_update($e, $mode = null, $columns = array(), $key_values = array())
	{
		return $this->make_cursor($this->sql()->update($mode, $columns, $key_values))->
				bind($e)->
	...
	public function raw_insert($e, $mode = null)
	{
		$rc = $this->make_cursor($this->sql()->insert($mode))->
				bind($e)->
				execute();
		if ($rc) {
			$id = (int)$this->connection->last_insert_id(reset($this->options['table']), $this->options['key']);
	...
	public function raw_delete($e, $mode = null)
	{
		return $this->make_cursor($this->sql()->delete($mode))->
				bind($e)->
				execute()->is_successful;
	}
	 ...
	 * @return DB_ORM_SQLMapper
	 */
	protected function apply_filter($args)
	{
		return $this;
	}
	...
	protected function callback($entity, $name)
	{
		...
		return is_object($entity) && method_exists($entity, $name) ?
			$entity->$name($result) && $result : $result;
	}
	...
	protected function make_cursor($sql, $typed = true)
	{
		...
		if ($typed) {
			$cursor->as_object($this->make_entity());
		}
		return $cursor;
	}
	...
	public function sql()
	{
		return new DB_ORM_SQLBuilder($this->options);
	}
...
class DB_ORM_SQLBuilder
{
	 ...
	 */
	public function __construct(DB_ORM_MappingOptions $options)
	{
		$this->for_options($options);
	}
	...
	public function update($mode = '', $columns = array(), $key_values = array())
	{
		$keys = array();
		foreach ($this->options['key'] as $v)
			$keys[] = "$v =  :" . (isset($key_values[$v]) ? 'key_' : '') . $v;
...
// TODO: deprecated
	 ...
	 * @return boolean
	 */
	public function __can_map($name, $is_call = false)
	{
...
abstract class DB_ORM_ConnectionMapper extends DB_ORM_MapperSet implements DB_ORM_ConnectionMapperInterface
{
	...
	{
		$this->active_connection = self::DEFAULT_CONNECTION_NAME;
		return $this;
	}
	...
	implements DB_ORM_EntityInterface /*, Core_CallInterface, IteratorAggregate*/, Core_StringifyInterface
{
	...
	{
		return $this->dispatch_res('after_insert', true);
	}

	public function after_update()
	{
		return $this->dispatch_res('after_update', true);
	}

	public function after_delete()
	{
		return $this->dispatch_res('after_delete', true);
	}

	public function after_save()
	{
		return $this->dispatch_res('after_save', true);
	}

	public function before_insert()
	{
		return $this->dispatch_res('before_insert', true);
	}

	public function before_update()
	{
		return $this->dispatch_res('before_update', true);
	}

	public function before_delete()
	{
		return $this->dispatch_res('before_delete', true);
	}

	public function before_save()
	{
		return $this->dispatch_res('before_save', true);
	}

	public function after_find()
	{
		return $this->dispatch_res('after_find', true);
	}

	public function after_make()
	{
		return $this->dispatch_res('after_make', true);
	}
	...
	public function  __call($method, $args)
	{
		switch (count($args)) {
			case 1:
				return $this->dispatch_res($method, null, null, $args[0]);
			case 2:
				return $this->dispatch_res($method, null, null, $args[0], $args[1]);
			default:
				return $this->dispatch_res($method, null, null);
		}
	}
