...
class MIME implements Core_ModuleInterface
{
	 ...
	 * @return string
	 */
	static public function boundary()
	{
		return '=_' . md5(microtime(1) . self::$boundary_counter++);
	}
	 ...
	 * @return string
	 */
	static public function default_charset()
	{
	 ...
	 * @return boolean
	 */
	static public function is_printable($string)
	{
		return (boolean)$string == '' || preg_match('{^[\x20-\x7e]+$}', $string);
	}
	...
	static public function encode($text, $encoding = MIME::ENCODING_B64)
	{
		switch ($encoding) {
			case MIME::ENCODING_B64:
				return self::encode_b64($text);
			case MIME::ENCODING_QP:
				return self::encode_qp($text);
			case MIME::ENCODING_8BIT:
				return $text;
			default:
				throw new MIME_UnsupportedEncodingException($encoding);
		}
	}
	...
	static public function decode($text, $encoding = MIME::ENCODING_B64)
	{
		switch ($encoding) {
			case MIME::ENCODING_B64:
				return self::decode_b64($text);
			case MIME::ENCODING_QP:
				return self::decode_qp($text);
			case MIME::ENCODING_8BIT:
				return $text;
			default:
				throw new MIME_UnsupportedEncodingException($encoding);
	 ...
	 * @return string
	 */
	static public function decode_b64($text)
	{
	 ...
	 * @return string
	 */
	static public function decode_qp($text)
	{
