...
class Text_Insertions_Filter implements Text_Insertions_FilterInterface
{
	...
	protected function replace_callback($m)
	{
		...
		$modifer = strtolower($m[2]);
		list($str, $args) = $this->standartizate_parms($m[3]);
		$result = $this->replace($name, $str, $args, $m[0], $modifer);
		if (!empty($result)) {
			//$result = " $result ";
		}
		return $result;
	...
	protected function replace($name, $str, $args, $original_str, $modifer)
	{
		if ($modifer) {
			$args['modifer'] = $modifer;
		}
		$res = null;
		Events::call("cms.insertions.$name", $str, $res, $args, $original_str, $this->args);
		if (is_string($res)) {
			return $res;
		}
		Events::call("%{$name}{{$str}}", $res, $args, $original_str, $this->args);
		if (is_string($res)) {
			return $res;
		}
		Events::call("cms.insertions", $name, $str, $res, $args, $original_str, $this->args);
	...
	public function add_views_paths($paths)
	{
		...
		$this->views_paths = array_unique(array_merge($this->views_paths, $paths));
		return $this;
	}
