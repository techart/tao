...
class Text_Parser_Wiki implements Core_ModuleInterface, Text_Process_ProcessInterface
{
	...
	public function parse_link_callback($m)
	{
		...
		if ($m = Core_Regexps::match_with_results('{^(.+)\|(.+)$}', $s)) {
			...
			$url = $s;
			$txt = $s;
			if (mb_strlen($txt) > self::$show_url_length + 3) {
				$txt = mb_substr($txt, 0, self::$show_url_length) . '...';
			}
		}
		$link_class = 'wiki_exists';
		if (Core_Regexps::match('{^http://}', $url)) {
			return sprintf(self::$template_external_link, $url, $txt);
		}
		if (Core_Regexps::match('{/}', $url)) {
			return sprintf(self::$template_link, $url, $link_class, $txt);
		}
		$wurl = false;
	...
	{
		$line = '';
		while (!$this->eof() && !($m = Core_Regexps::match_with_results('{^\{\|(.*)}', $line)))
			$line = trim($this->get());
		if ($this->eof())
			return '';
		$parms = trim($m[1]);
