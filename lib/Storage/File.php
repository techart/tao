...
abstract class Storage_File_Type extends Storage_Type {
	...
	public function remove_from_index($e) {
		...
		if (!empty($index['ids'])) {
			...
			if ($key !== FALSE) unset($index['ids'][$key]);
			return $this->write($this->path_to_index(), $index);
		}
		return true;
	}
	...
	public function generate_id() {
		$index = $this->read($this->path_to_index());
		if (empty($index) || empty($index['ids'])) return 1;
	...
	protected function query_cache($key, $value = null) {
		...
		if (is_null($value)) return $this->read($path);
		else return $this->write($path, $value);
	}

	public function query_cache_path($key = '') {
		$base = $this->base_path() . DIRECTORY_SEPARATOR . 'query_cache';
		if (!IO_FS::exists($base)) IO_FS::make_nested_dir($base);
	...
	public function select($query = null) {
		...
		if ($query && $this->use_query_cache && $cache = $this->get_query_cache($query)) return $cache; 
		$res = $this->select_by_ids($this->get_ids());
		if ($query) {
			$res = $query->execute($res);
			if ($this->use_query_cache) $this->set_query_cache($query, $res);
		}
		return $res;
	}
