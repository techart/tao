...
class CMS_Controller_Base extends WebKit_Controller_AbstractController implements Core_ModuleInterface
{
	...
	{
		if ($this->auth_realm_title) {
			return $this->auth_realm_title;
		}
		return $this->auth_realm();
	}
	...
	public function setup()
	{
		...
		if (CMS::$default_last_modified) {
			$this->response['Last-Modified'] = gmdate("D, d M Y H:i:s T");
		}
	...
	{
		if (is_callable(CMS::$after_auth)) {
			return call_user_func(CMS::$after_auth, $parms);
		}
	}
