...
class CMS_Controller_AdminVars extends CMS_Controller implements Core_ModuleInterface { 
	...
	public function setup() {
		...
		if (isset(CMS::$cfg->vars->use_tinymce)) {
			...
			if ($s==''||$s=='0') $this->use_tinymce = false;
		}	
	...
	public function index($parms) { 
		$id = (int)$parms['id']; 
		if ($id==0) return $this->dir(0,$parms['component']); 
		if ($id>0) { 
			$this->_item = CMS::vars()->db()->find($id); 
			if ($this->_item->vartype=='dir') return $this->dir($id,$parms['component']); 
			else { 
	...
	protected function get_title($item) {
		...
		if ($component_class_name) {
			...
			if (!$title) $title = "Настройки: $component_class_name";
			if ($item->id>0&&trim($item->title)!='') $title .= ': '.$item->title;
		}
		return $title;
	}
	...
	public function imagelist($parms) { 
		...
		echo 'var tinyMCEImageList = new Array('.implode(',',$ar).');'; die; 
	} 
	...
	public function change($parms) { 
		...
		else return $this->redirect_to($this->make_uri('dir',$item->parent_id)); 
	} 
