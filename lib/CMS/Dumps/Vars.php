...
class CMS_Dumps_Vars extends CMS_Dumps_AbstractDumper implements Core_ModuleInterface {
	...
	static function load($component) {
		...
		while($line = self::get()) {
			$line = trim($line);
			if ($line!='') {
				if ($line=='!ENDDUMP') return;
				else if ($line=='!BEGIN') self::load_var();
				else if ($m = Core_Regexps::match_with_results('{^!FILE\s+(\d+)(.+)$}',$line)) {
					$id = trim($m[1]);
					$filename = trim($m[2]);
					if ($filename=='') $filename = rand(11111,99999);
	...
	static function load_var() {
		...
		while($line = self::get()) {
			...
			if ($line!='') {
				...
				else if ($m = Core_Regexps::match_with_results('{^([^\s:]+):(.*)$}',$line)) {
					$key = trim($m[1]);
					$value = trim($m[2]);
					$value = $value=='!BEGIN'? self::decode_content() : base64_decode($value);
					if ($key=='site'&&$value=='__') $value = CMS_Admin::site();
					$var->$key = $value;
				}
			}	
		}	
	...
	static function load_file($oid,$filename) {
		$id = self::$ids[$oid];
		if ($id==0||$oid==0) return;
		
		$dir = "../".CMS::$www."/".Core::option('files_name')."/vars/$id";
		if (!IO_FS::exists($dir)) IO_FS::make_nested_dir($dir,CMS::$chmod_dir);
	...
	static function dump($component) { 
		...
		$filename = 'vars';
		if ($component!='') $filename .= "-$component";
