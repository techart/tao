...
class CMS_ExceptionCatcher implements Core_ModuleInterface {
	...
	static function send($e) {
		...
		if ($email) {
			...
			if (!$project) $project = $_SERVER['SERVER_NAME'];
	...
	static function dump($array,$title) {
		...
		while (strlen($t)<50) $t .= '-';
		print "$t\n";
		if (sizeof($array)==0) print "empty\n";
		else foreach($array as $k => $v) {
			print "$k: ";
			if (is_array($v)||is_object($v)) print base64_encode(serialize($v));
			else print $v;
			print "\n";
		}
		print "\n";
	...
	static function render_value($value) {
		...
		if (is_array($value)) return '['.self::render_array($value).']';
		return '???';
	}
} 
