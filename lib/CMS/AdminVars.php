...
class CMS_AdminVars implements Core_ModuleInterface {
        ...
        static function option($name, $value = null) {
          $path = explode('/', $name);
          $v = null;
          foreach ($path as $p) $v = self::$options[$p];
          if (is_null($value)) return $v;
          else self::options(array($name => $value));
        }
...
class CMS_AdminVars_TreeMapper extends CMS_Mapper {
	...
	protected function get_component_name($request) {
		...
		if (preg_match($regexp,$uri,$m)) {
			...
			if (trim($m[1])!='') {
				$component = trim($m[1]);
				if (!CMS::component_exists($component)) return false;
...
class CMS_AdminVars_Mapper extends WebKit_Controller_AbstractMapper {
	...
	public function route($request) {
		...
		if (preg_match('{^'.CMS::admin_path().'vars(?:-([^/]+))?/(.*)$}',$uri,$m)) {
			...
			if ($path=='') return array('controller' => $ctr, 'action' => 'index', 'id' => 0,'component'=>$component);
			if (preg_match('{^(\d+)$}',$path,$m)) return array('controller' => $ctr, 'action'=>'index','id'=>$m[1],'component'=>$component);
			if ($path=='loaddump') return array('controller' => $ctr, 'action'=>'loaddump');
			if ($path=='dump') return array('controller' => $ctr, 'action'=>'dump','component'=>$component);
			if (preg_match('{(dumpvar|add|del|parms|chparms|change|addfile|delfile|imagelist|attaches)/(\d+)}',$path,$m)) return array('controller' => $ctr, 'action'=>$m[1],'id'=>$m[2],'component'=>$component);
			return false;
		}
		return false;
	}
	...
	public function make_uri() {
		...
		if ($args[0]=='dir') return CMS::admin_path()."$path/".$args[1];
