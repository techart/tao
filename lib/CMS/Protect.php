...
class CMS_Protect implements Core_ModuleInterface
{
	...
	public function draw($page = '')
	{
		...
		if ($foreground_color === false) {
			$foreground_color = array(0, 0, 0);
		} else {
			$foreground_color = self::convert_color($foreground_color);
		}
		if ($background_color === false) {
			$background_color = array(200, 200, 200);
		} else {
			$background_color = self::convert_color($background_color);
		}
	...
	static function hex_color($color)
	{
		...
		if ($color == '') {
			...
			if (strlen($color) == 3) {
				...
				if (strlen($color) != 6) {
					$color = $color[0] . '0' . $color[0] . '0' . $color[0] . '0';
				}
			}
		}
	...
	static function convert_color($color)
	{
		if (is_string($color)) {
			$color = trim($color);
			if ($m = Core_Regexps::match_with_results('{^#([0-9a-f]+)$}i', $color)) {
