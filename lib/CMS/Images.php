...
///   </protocol>
	...
	static function get_image_size($filename) {
		if (isset(self::$gisz[$filename])) return self::$gisz[$filename];
	...
	static function string2sizes($src) {
		...
		if ($src!='') {
			...
			else if ($m = Core_Regexps::match_with_results('{(\d+)x}i',$src)) {
				$w = (int)$m[1];
			}
...
class CMS_Image {
	...
	public function resize($w,$h,$fit=true) { return $this; }
	...
	public function save($file,$format='jpg') {
		file_put_contents($file,$this->image($format));
		return $this;
	}
	...
	protected function watermark_parms(&$parms, $_w, $_h, $w, $h) {
		...
		if(is_array($parms)) {
			if(preg_match('{^[1-9]$}', $parms['mode'])) {
				if($parms['mode'] <= 3) $nt = $_h-$h;
				if($parms['mode'] >= 4 && $parms['mode'] <= 6) $nt = (int)(($_h-$h)/2);
				if(!($parms['mode']%3)) $nl = $_w-$w;
				if(!(($parms['mode']+1)%3)) $nl = (int)(($_w-$w)/2);
			} else {
			...
			if(isset($parms['opacity'])) $opacity = (float)$parms['opacity']; else $opacity = 0.2;
		}
...
class CMS_Image_GD extends CMS_Image {
	...
	public function save($file,$format=false) {
		if (!$format) $format = $this->loaded_format;
	...
	protected function color2int($c) {
		...
		if(is_int($c)) $nc = $c;
		elseif($ac) $nc = $ac[0]*256*256 + $ac[1]*256 + $ac[2];
	...
	public function watermark($image = false, $parms = false) {
		...
		if ($opacity==1) @imagecopy($this->ih,$watermark,$nl,$nt,0,0,$w,$h);
		else @imagecopymerge($this->ih,$watermark,$nl,$nt,0,0,$w,$h,(int)($opacity*100));
...
class CMS_Image_Imagick extends CMS_Image {
	...
	public function watermark($image = false, $parms = false) {
		...
		if ($opacity!=1) $watermark->setImageOpacity($opacity);
