...
class CMS_Fields_Types_Image extends CMS_Fields_AbstractField implements Core_ModuleInterface
{
	...
	protected function upload_validate($name, $data, $file, $new)
	{
		...
		if (!$this->is_valid($realname)) {
			return CMS::lang()->_common->ta_image_error;
		}
	...
	public function uploaded_file_name($name, $data, $item, $ext, $filename)
	{
		if (isset($data['inserted_filename'])) {
			$callback = $data['inserted_filename'];
			$result = call_user_func($callback, $filename, $name, $data, $item);
			if ($result) {
				return $result;
			}
		}
...
abstract class CMS_Fields_Types_Image_ModsCache extends CMS_Fields_ValueContainer
{
	...
	public function fit()
	{
		...
		unset($mod['color']);
		if (isset($mod['width']) && isset($mod['height'])) {
			$this->mods[] = $mod;
		}
		return $this;
	}
	...
	protected function mods_path()
	{
		...
		if ($m = Core_Regexps::match_with_results('{/([^/]+)$}', $original)) {
			$original_name = $m[1];
		}
		...
		foreach ($this->mods as $mod) {
			if (isset($mod['action'])) {
				...
				$filename .= $action;
				if (isset($mod['width']) && isset($mod['height'])) {
					$filename .= $mod['width'] . 'x' . $mod['height'];
				}
				if (isset($mod['color'])) {
					$filename .= strtolower(str_replace('#', '', trim($mod['color'])));
				}
				if (isset($mod['image'])) {
					$_s = $mod['image'];
					$this->extra_file = $_s;
...
class CMS_Fields_Types_Image_Container extends CMS_Fields_Types_Image_ModsCache
{
	...
	public function value()
	{
		...
		if ($value == '' && isset($this->data['default_image'])) {
	...
	{
		if (isset($this->data['default_image'])) {
			return trim($this->value_to_path($this->data['default_image']));
		}
		return '';
	}
