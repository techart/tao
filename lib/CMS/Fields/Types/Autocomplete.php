...
class CMS_Fields_Types_Autocomplete extends CMS_Fields_AbstractField implements Core_ModuleInterface {
	...
	protected function get_empty($data) {
		if (isset($data['empty'])) {
			if ($data['empty'] == false) return false;
			if (is_string($data['empty'])) return array('id' => 0, 'title' => '---');
			return $data['empty'];
		}
		return $this->empty;
	}
	...
	protected function get_item_text($name, $data) {
		...
		if ($empty && $empty['id'] == $value) return $empty['title'];
	...
	protected function layout_preprocess($l, $name, $data) {
		...
		$l->use_scripts(CMS::stdfile_url('scripts/ext-all.js'));
		$l->use_scripts($js_file);
		$l->use_style(CMS::stdfile_url('styles/custom-ext-theme.css'));
	...
	protected function preprocess($t, $name, $data) {
		...
		$data['tagparms']['data-href'] = CMS::$current_controller->field_action_url($name, 'items', $data['__item']);
		$data['tagparms']['data-type'] = 'autocomplete';
		$data['tagparms']['data-disable'] = (int) $js_file == $this->js_file;
		
		$t->append_to('js',
		"$( function() {
			Ext.select('input[data-type=autocomplete]:not(.field-process)').filter('[data-disable!=1]').each(function(input) {
				input.addCls('field-process');
				TAO.fields.autocomplete(input);
			});
		});"
		);
	...
	public function action($name, $data, $action, $item = false, $fields = array()) {
		...
		$values = array();
		$empty = $this->get_empty($data);
		if ($empty && empty($query)) $values[] = $empty;
		foreach ($ents as $k => $e) {
