...
class CMS_ORM_Mapper extends DB_ORM_SQLMapper
{
	...
	public function setup_config()
	{
		...
		if ($component && $component->schema_modules()) {
			$modules = $component->schema_modules();
			$name = self::table_from($this);
			if (isset($modules[$name])) {
				$this->schema_fields($modules[$name]);
			}
		}
	...
	public function auto_add_field_created($v = null)
	{
		...
		if (is_null($v)) {
			return $this->option('auto_add_field_created');
		}
	...
	public function auto_add_field_time($v = null)
	{
		...
		if (is_null($v)) {
			return $this->option('auto_add_field_time');
		}
	...
	public function auto_add_lifetime($v = null)
	{
		...
		if (is_null($v)) {
			return $this->option('auto_add_lifetime');
		}
	...
	public function fields()
	{
		...
		return $data ? array_replace_recursive($this->fields_data, $data) : $this->fields_data;
	}
...
class CMS_ORM_Entity extends DB_ORM_Entity implements DB_ORM_AttrEntityInterface
{
	...
	public function before_save()
	{
		...
		if ($this->clear_cache) {
			...
			if ($dir = $this->cache_dir_path(true)) {
				CMS::rmdir($dir);
			}
		}
	...
	protected function unserialize_parms()
	{
		...
		foreach ($args as $arg) {
			if (is_string($this->$arg)) {
				$this->$arg = unserialize((string)$this->$arg);
			}
			if (!$this->$arg) {
				$this->$arg = array();
			}
...
//TODO: merge whi associated in DB.ORM
	...
	{
		foreach ($this->multilinks() as $mapper)
