...
class Templates_HTML_Helpers_Forms implements Core_ModuleInterface, Templates_HelperInterface, Core_PropertyAccessInterface
{
	...
	{
		return new Templates_HTML_Helpers_Forms();
	}
	 ...
	 * @param Templates_HTML_Template $t
	 */
	public function end_fieldset(Templates_HTML_Template $t)
	{
		return "</fieldset>";
	}
	...
	public function begin_field(Templates_HTML_Template $t, $name, array $attrs = array())
	{
		...
		if ($this->form->validator &&
			$this->form->validator->errors->has_error_for($name)
		) {
			$attrs['class'] .= ' error';
		}
	...
	public function fields(Templates_HTML_Template $t)
	{
		...
		foreach ($args as $field)
			$r .= call_user_func_array(array($this, 'field'), array_merge(array($t), $field));
		return $r;
	}
	...
	public function begin_form_tag(Templates_HTML_Template $t, $action, $method, array $attributes = array())
	{
		...
		$result = $t->tag('form', Core_Arrays::merge(array(
						'action' => $action,
						'method' => ($method == 'post' || $method == 'get' ? $method : 'post')), $attributes
				), false
			) . "\n";
	 ...
	 * @return string
	 */
	public function end_form_tag(Templates_HTML_Template $t)
	{
		return "</form>\n";
	}
	 ...
	 * @return string
	 */
	protected function field_name_for($name)
	{
	 ...
	 * @return string
	 */
	protected function field_id_for($name)
	{
		return "{$this->form->name}_$name";
	}
