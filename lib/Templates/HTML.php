...
class Templates_HTML implements Core_ConfigurableModuleInterface
{
	...
	static protected $use_files = array();
	...
	public static function file_to_docroot($file, $dest = null, $copy_dir = '', $link = false)
	{
		return self::extern_filepath($file, true, $link, $dest, $copy_dir);
	}

	/**
	 * @param stirng $path
	 */
	static public function extern_filepath($file, $copy = true, $link = false, $dest = null, $copy_dir = null)
	{
		$file = (string)$file;
	 ...
	 */
	static public function helpers()
	{
		return self::$helpers;
	}
	...
	{
		if (is_object(self::$meta)) {
			return self::$meta;
		}
		return self::$meta = new Templates_HTML_Metas();
	}

	public static function use_file($name, $parms = array())
	{
		$parms['name'] = $name;
		self::$use_files[$name] = $parms;
	}

	public static function get_use_files()
	{
	...
	implements Core_IndexedAccessInterface
{
	...
	{
		$this->setup_agregators($js_agregator, $css_agregator);
		parent::__construct($name);
		$this->content = new ArrayObject();
		$this->with($parameters);
	}

	protected function setup_agregators($js_agregator = null, $css_agregator = null)
	{
	...
	protected function setup($js_agregator = null, $css_agregator = null)
	{
		if ($this->enable_less) {
			$this->enable_less();
		}
	...
	{
		$this->root->build_head();
		return array(
			'data' => $this->render(),
			'eval' => trim($this->root['js']),
			'js' => $this->js_agregator->files_list(),
			'css' => $this->css_agregator->files_list()
		);
	}
	...
	public function include_files($place = null)
	{
		$res = '';
		...
		foreach ($this->agregators as $type => $a) {
			$method = $type . '_link';
			foreach ($a->files_list($place) as $file_path => $file) {
				...
				$absolute = !empty($file) && isset($file['absolute']) ? $file['absolute'] : null;
				if (method_exists($this, $method)) {
					$res .= $this->$method($file_path, $options, $add_timestamp, $absolute);
				} else {
					$res .= $this->get_helpers()->$method($file_path, $options, $add_timestamp, $absolute);
				}
			}
		}
		return $res;
	...
	public function is_layout($v = null)
	{
		if (is_null($v)) {
	 ...
	 * @return Templates_HTML_Template
	 */
	public function prepend_to($name, $content)
	{
		return $this->content($name, $content, true);
	}
	 ...
	 * @return Templates_HTML_Template
	 */
	public function append_to($name, $content)
	{
		return $this->content($name, $content, false);
	}
...
//-----------------------
	...
	{
		return $this->if_empty('title', htmlspecialchars($content));
	}
	...
	{
		return $this->if_empty('description', htmlspecialchars($content));
	}
...
//-----------------------
	 ...
	 * @return string
	 */
	public function js_link($path, array $options = array(), $add_timestamp = true, $absolute = null)
	{
		return $this->content_tag('script', '',
			array_merge(array(
					'src' => $this->js_path($path, $add_timestamp, $absolute)
				),
				$options
			)
		) . "\n";
	 ...
	 * @return string
	 */
	public function css_link($path, array $options = array(), $add_timestamp = true, $absolute = null)
	{
		return $this->tag('link',
			array_merge(array(
					'rel' => 'stylesheet',
					'type' => 'text/css',
					'href' => $this->css_path($path, $add_timestamp, $absolute)
				),
				$options
			)
		) . "\n";
	...
	protected function js_path($path, $add_timestamp = true, $absolute = null)
	{
	...
	{
		if ((
				(is_null($absolute) && Templates_HTML::option('absolute_urls'))
				|| $absolute
			)
			&& !Templates_HTML::is_url($path)
		) {
	...
	public function __sleep()
	{
		return array_merge(parent::__sleep(),
			array('include_meta', 'allow_filtering', 'enable_less', 'views_paths')
		);
	}
	...
	protected function render_nested(ArrayObject $content = null)
	{
		...
		$this->render_self();
		if (!empty($this->spawn_from)) {
			$this->spawn_from->update_content($this->content, array('content'));
		}
		$rc = $this->container ?
	...
	protected function filter_required($content)
	{
		...
		if (!$filter->exists('assets')) {
			$filter->register(array('assets' => new Core_Call($this, 'include_files')));
		}
	...
	Core_StringifyInterface
{
	...
	{
		$this->{'msapplication-config'}('none');
		Events::call('templates.metas.create', $this);
	}
	...
	public function __unset($property)
	{
		...
		if ($this->is_http_field($property)) {
			if (isset($this->http[$property = $this->http_field_name($property)])) {
				unset($this->http[$property]);
			} else {
				if (isset($this->named[$property])) {
					unset($this->named[$property]);
				}
			}
		}
	}
