...
class Tree_DB_MPT implements Core_ModuleInterface
{
	...
	static public function schema_join(&$data, $table = null)
	{
		$data = array_merge_recursive($data, array(
				...
				'indexes' => array(
					...
					"idx_{$table}_entity_id" => array('columns' => array('entity_id')),
					"idx_{$table}_parent_id" => array('columns' => array('parent_id')),
					"idx_{$table}_level" => array('columns' => array('level')),
					"idx_{$table}_ord" => array('columns' => array('ord')),
				)
			)
		);
	}
...
class Tree_DB_MPT_Mapper extends Tree_DB_AbstractMapper
{
	...
	protected function map_descendants_all($items)
	{
		$ids = array();
		...
		return $this->map_join_tree()->in($this->option('tree_alias') . '.parent_id', $ids)->map_sort_tree();
	}
...
class Tree_DB_MPT_EntitySubscriber extends Tree_DB_EntitySubscriber
{
	...
	{
		if ($e->obj->old_parent_id == $e->obj->parent_id) {
			return true;
		}
