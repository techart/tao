...
class Tree_DB_MP implements Core_ModuleInterface
{
	...
	static public function schema(&$data)
	{
		...
		$data['indexes']["idx_{$table}_path"] = array('columns' => array(array('path', 255)));
		$data['indexes']["idx_{$table}_path1"] = array('columns' => array(array('path1', 255)));
		$data['indexes']["idx_{$table}_level"] = array('columns' => array('level'));
		$data['indexes']["idx_{$table}_parent_id"] = array('columns' => array('parent_id'));
		$data['indexes']["idx_{$table}_ord"] = array('columns' => array('ord'));
	}
...
class Tree_DB_MP_EntitySubscriber extends Tree_DB_EntitySubscriber
{
	...
	{
		if ($e->obj->manual_setup || $e->obj->old_parent_id == $e->obj->parent_id) {
			return true;
		}
		$parent = $e->obj->parent;
