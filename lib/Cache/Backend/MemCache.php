...
class Cache_Backend_MemCache_Backend extends Cache_Backend
{
	...
	{
		$list = array(); 
		$slabs_by_server = $this->memcache->getExtendedStats('slabs'); 
		foreach($slabs_by_server as $server => $slabs) {
			if (empty($slabs)) {
				continue;
			}
			foreach($slabs AS $slab_id => $slab_meta) {
				if (!is_int($slab_id) || !is_array($slab_meta)) {
					continue;
				}
				$cdump = $this->memcache->getExtendedStats('cachedump', (int) $slab_id, 1000);
				if ($cdump && isset($cdump[$server]) && is_array($cdump[$server]) && !isset($cdump[$server]['CLIENT_ERROR'])) {
					$keys = array_keys($cdump[$server]);
					if ($keys) {
