...
class WebKit_Controller implements Core_ModuleInterface
{
	...
	{
		return new WebKit_Controller_Mapper();
	}
	...
	{
		return new WebKit_Controller_Route();
	}
	...
	Core_IndexedAccessInterface
{
	...
	{
		return isset($this->parms[$property]);
	}
	...
	implements Core_IndexedAccessInterface
{
	 ...
	 * @return WebKit_Controller_AbstractMapper
	 */
	public function use_options(array $options)
	{
		return $this->with_options($options);
	}
	 ...
	 * @return WebKit_Controller_AbstractMapper
	 */
	public function use_defaults(array $defaults)
	{
		return $this->with_defaults($defaults);
	}
	 ...
	 * @return string
	 */
	protected function add_path($url)
	{
		return $this->options['path'] . $url;
	}
	...
	implements Core_PropertyAccessInterface, Core_CallInterface
{
	...
	/**
	 * @param string|Templates_Template $template
	 * @param array                     $parms
	 * @param array                     $response_data
	 *
	 * @return Net_HTTP_Response
	 */
	protected function render_json($template, array $parms = array(), array $response_data = array())
	{
		if (!($template instanceof Templates_Template)) {
			$template = $this->render((string)$template, $parms);
		}
		return $this->json_response(array_replace_recursive($template->response_data(), $response_data));
	}
	 ...
	 * @return Net_HTTP_Response
	 */
	protected function json_response($response_data, $response = '')
	{
		return Net_HTTP::Response($response)->content_type('application/json')->body(json_encode($response_data));
	}
	...
	{
		foreach ($args = func_get_args() as $script)
	...
	public function dispatch(WebKit_Controller_Route $route)
	{
		...
		if ($result === null) {
			$result = $this->render($route['action']);
		}
